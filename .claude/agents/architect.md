...
1. **enum-based dispatch**：4 backend (Local/NFS/S3/HDFS) 在 `src/storage_enum.rs` 用 match 分派；CIFS 只有 role-based 实现，不在 enum 里。改一个 `StorageEnum` 操作 = `storage_enum.rs` + 4 个 backend.rs + 可能 lib.rs + example + test。
...
4. **资源句柄走 `close_resource`** (storage/backends/cifs/protocol.rs)：error path 不漏 close。
...
- src/hdfs.rs: 客户端不暴露 truncate → 返回 UnsupportedType
...
- MEDIUM: CIFS Samba 和 Windows server 实现 SetFileInfo 行为不一致。先在 .env CIFS_REAL_SERVER 上测 Samba，再加 Win server 测试到 backlog。
