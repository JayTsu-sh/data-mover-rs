...
缓解分三层，**逐目录动态触发** (只有流着才知道这个目录有多少子目录)：

#### 第 0 层：名字 arena (恒定生效，[计划] 随 P9)
...
约 7 倍。**这一层把切换门槛抬得很高** —— 几百 MB 在多数部署里是可以接受的，于是下面两层
根本不会触发，契约、预取、句柄生命周期全部不受影响。

#### 第 1 层：arena 溢写 (默认逃生门，[计划] 复用 P8 的溢写)
...
if global_slot_bytes > BUDGET:
    候选 = 槽位字节最多的帧
    if 候选.closed → 只能溢写 (层 1)     ← 已无流可停，换策略救不了
    else           → 切窗口 (层 2)
```
...
**读完流才能放掉目录句柄，但读完流就必须记下全部槽位。** 层 2 选择了不读完，于是该目录的
列举游标要一直开到整棵子树遍历结束 —— 1000 万子目录可能是数小时。

这个问题的严重性**完全取决于协议的"位置"模型**：

| backend | park 期间要持有什么 | 层 2 可用性 |
|---|---|---|
...
3. **在 CIFS 上不提供层 2** —— 无盘且 arena 放不下时**拒绝该目录并返回 typed 失败**。

**推荐第 3 条。** 理由：层 2 在 CIFS 上大概率半路失败，而**半路失败比一开始拒绝糟糕得多** ——
那时已经遍历了几小时。注意由此得到的对称性：**层 2 只在句柄不成问题的 backend 上可用**
(NFS / S3 / Local)，而 CIFS 恰好既是句柄最脆的、也是最该走层 0 + 层 1 的。

#### 三层的取舍

| 层 | 内存 | 契约 | 句柄 | 需要 |
|---|---|---|---|---|
| 0 arena | ~7× 改善 | 全保住 | 读完即释放 | — |
| 1 溢写 | 常数 | 全保住 | **读完即释放** | 本地磁盘 |
| 2 窗口 K | 常数 | **时序退化** | **持有整棵子树** | — (CIFS 除外) |

优先级是 **0 → 1 → 2**，且层 1 与层 2 不是等价备选：**层 1 唯一的代价是要盘，换来契约与句柄
双全**；层 2 省掉磁盘，代价是时序退化**加上**句柄长持有。

**`NameBytes` 下只有第 0 / 1 层。** 排序本来就要物化整块，层 2 在语义上不成立 —— 没看完整块
